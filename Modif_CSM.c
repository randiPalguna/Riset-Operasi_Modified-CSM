...
void findDonatur(int * donaturRow, int * receiverRow, int * donationCol) {
  ...
  for (int j = 0; j < col; j++) {
    if (localSmallest[j] != INT_MAX) {
      printf("Diff Kolom %d = %d\n", j + 1, localSmallest[j]);
    } else {
      printf("Diff Kolom %d = NULL\n", j + 1);
    }
  }
  printf("\n");

  printf("STEP 5: Tentukan Kolom Smallest Diff\n");
  //donasi paling optimal
  if (!flag) { // no tie, only 1 SDiff
    printf("Kondisi tidak ada TIE\n");
    printf("Kolom smallest dif berada pada kolom %d\n\n", smallestIndex + 1);
    * donaturRow = localDon[smallestIndex];
    * receiverRow = localRec[smallestIndex];
    * donationCol = smallestIndex;
  } else { // more than 1 SDiff
    printf("Kondisi ada TIE\n");
...
void donate(int * donaturRow, int * receiverRow, int * donationCol) {
  ...
  printf("Jumlah unit yang dipindah dari FLC ke SLC = %d\n\n", maxDon);
...
void compute() {
  ...
  check();
  while (satisfied) {
    printf("=== Iterasi ke-%d ===\n", iteration++); // <-- print iterasi di sini
    donaturRow = -1;
    receiverRow = -1;
